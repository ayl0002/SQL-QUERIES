...
FROM #FINAL B
LEFT JOIN (SELECT LOAN_NBR,MAX(SM_COUNT) AS 'Slow_Mover_Count' FROM tbl2_SM GROUP BY LOAN_NBR) C
	ON B.LOAN_NBR = C.LOAN_NBR
LEFT JOIN (SELECT LOAN_NBR,MAX(Aging) AS 'Slow_Mover_Aging' FROM tbl2_SM GROUP BY LOAN_NBR) D
	ON B.LOAN_NBR = D.LOAN_NBR
LEFT JOIN tbl3 E
	ON CAST(B.Loan_Nbr AS VARCHAR) = CAST(E.[Loan Number] AS VARCHAR)
LEFT JOIN (SELECT LOAN_NBR,SUM(DAYS_TO_CLOSE) AS 'EXCP_AGING' FROM tbl1 GROUP BY LOAN_NBR) F
	ON B.LOAN_NBR = F.LOAN_NBR
LEFT JOIN  tbl2_SM H
	ON B.LOAN_NBR = H.LOAN_NBR


SET NOCOUNT ON SELECT B.*,D.[Tag 2],D.[Incurable Flag] FROM tbl1 B LEFT JOIN tbl3 D ON CAST(B.Loan_Nbr AS VARCHAR) = CAST(D.[Loan Number] AS VARCHAR)

SET NOCOUNT ON SELECT b.*,D.[Tag 2],D.[Incurable Flag] FROM tbl2_SM B LEFT JOIN tbl3 D ON CAST(B.Loan_Nbr AS VARCHAR) = CAST(D.[Loan Number] AS VARCHAR)
