...
INTO #BASE
FROM #BASE0 A
LEFT JOIN (SELECT LOAN_NBR,[STG_VAL],[FINAL_RVW_STS_DESC],[INITL_RVW_STS_DESC],[TAG_2_VAL],[INCRBL_FLG_DESC],GRP_DESC FROM tbl5 WHERE CURR_IND = 'Y') B
...
LEFT JOIN (SELECT LOAN_NBR FROM tbl2 WHERE [DOC_DESC] = 'Loss Draft' AND CURR_IND = 'Y' AND [EXCP_STS_DESC] NOT IN ('RESOLVED','CLOSED','NOT VALID','CLOSED WITH VENDOR')) B
	ON BASE.LOAN_NBR = B.LOAN_NBR
LEFT JOIN (SELECT LOAN_NBR FROM tbl2 WHERE [ISSU_DESC] = 'Forced Placed Insurance' AND CURR_IND = 'Y' AND [EXCP_STS_DESC] NOT IN ('RESOLVED','CLOSED','NOT VALID','CLOSED WITH VENDOR')) C
	ON BASE.LOAN_NBR = C.LOAN_NBR
LEFT JOIN (SELECT LOAN_NBR,[HUD_STS_DESC] FROM tbl3 WHERE CURR_IND = 'Y') D
