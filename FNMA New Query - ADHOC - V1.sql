...
SET NOCOUNT ON
SELECT A.*,C.* INTO #BASE FROM tbl1 A
	LEFT JOIN (SELECT * FROM tbl2 WHERE [EXCEPTION ID] IS NOT NULL) C
...
LEFT JOIN (SELECT [EXCEPTION ID],(SELECT COUNT(V) FROM (VALUES ([Gift Card Letter Sent]),([Gift Card Letter Sent 2]),	([Gift Card Letter Sent 3]),([Ledger Letter Sent 1]),	([Ledger Letter Sent 2]),	([Ledger Letter Sent 3]),	([Non GC Letter Sent 1]),	([Non GC Letter Sent 2]),	([Non GC Letter Sent 3])) AS VALUE (V)) 
AS [HOA_LTRS]
FROM #BASE WHERE [Document] IN ('HOA')) C
ON B.[EXCEPTION ID] = C.[EXCEPTION ID]

LEFT JOIN (SELECT [EXCEPTION ID],(SELECT COUNT(V) FROM (VALUES ([Gift Card Letter Sent]),([Gift Card Letter Sent 2]),	([Gift Card Letter Sent 3]),([Non GC Letter Sent 1]),	([Non GC Letter Sent 2]),	([Non GC Letter Sent 3])) AS VALUE (V)) 
AS 'OCC_LTRS'
FROM #BASE
WHERE [Document] IN ('Current OCC Cert')) D
ON B.[EXCEPTION ID] = D.[EXCEPTION ID]

LEFT JOIN (SELECT [EXCEPTION ID],(SELECT COUNT(V) FROM (VALUES ([Gift Card Letter Sent]),([Gift Card Letter Sent 2]),	([Gift Card Letter Sent 3])) AS VALUE (V)) 
AS 'DC_LTRS'
FROM #BASE
WHERE [DOCUMENT] IN ('Death Cert HACG')) E
ON B.[EXCEPTION ID] = E.[EXCEPTION ID]

LEFT JOIN (SELECT [EXCEPTION ID], COUNT([Sent For Gift Card Processing]) AS 'GC_COUNT' FROM #BASE WHERE [Sent For Gift Card Processing] IS NOT NULL AND [DOCUMENT] IN ('Death Cert HACG','Trust - HACG','Proof of Repair','Current OCC Cert','HOA') GROUP BY [EXCEPTION ID]) F
...
LEFT JOIN (SELECT [EXCEPTION ID],(SELECT COUNT(V) FROM (VALUES ([Ledger Letter Sent 1]),([Ledger Letter Sent 2]),([Ledger Letter Sent 3]),([sent to inspection vendor])) AS VALUE (V)) AS 'OCC_DK_COUNT'
FROM #BASE
WHERE [DOCUMENT] IN ('CURRENT OCC CERT')) H
ON B.[EXCEPTION ID] = H.[EXCEPTION ID]

LEFT JOIN (SELECT [EXCEPTION ID],(SELECT COUNT(V) FROM (VALUES ([Ledger Letter Document Returned]),	([sent to inspection vendor])) AS VALUE (V)) AS 'HOA_DK_COUNT'
FROM #BASE
...
LEFT JOIN (SELECT BB.LOAN_NBR,MAX(NTS_COUNT) AS 'NTS_COUNT' FROM tbl1 BB
			INNER JOIN (SELECT LOAN_NBR,TAX_VENDOR,ROW_NUMBER() OVER (Partition By Loan_Nbr ORDER BY TAX_VENDOR) AS 'NTS_COUNT' FROM tbl1 WHERE TAX_VENDOR = 'NTS') AA
			ON AA.LOAN_NBR = BB.LOAN_NBR  GROUP BY BB.LOAN_NBR) NTS
	ON NTS.LOAN_NBR = BASE.LOAN_NBR
LEFT JOIN (SELECT BB.LOAN_NBR,MAX(ISGN_COUNT) AS 'ISGN_COUNT' FROM tbl1 BB
			INNER JOIN (SELECT LOAN_NBR,TAX_VENDOR,ROW_NUMBER() OVER (Partition By Loan_Nbr ORDER BY TAX_VENDOR) AS 'ISGN_COUNT' FROM tbl1 WHERE TAX_VENDOR = 'ISGN') AA
			ON AA.LOAN_NBR = BB.LOAN_NBR GROUP BY BB.LOAN_NBR) ISGN
	ON ISGN.LOAN_NBR = BASE.LOAN_NBR
LEFT JOIN (SELECT BB.LOAN_NBR,MAX(LRES_COUNT) AS 'LRES_COUNT' FROM tbl1 BB
			INNER JOIN (SELECT LOAN_NBR,HOA_VENDOR,ROW_NUMBER() OVER (Partition By Loan_Nbr ORDER BY TAX_VENDOR) AS 'LRES_COUNT' FROM tbl1 WHERE HOA_VENDOR = 'Local Vendor') AA
			ON AA.LOAN_NBR = BB.LOAN_NBR GROUP BY BB.LOAN_NBR) LRES
	ON LRES.LOAN_NBR = BASE.LOAN_NBR

SELECT *,GC_EXPENSE+DK_EXPENSE+LTR_EXPENSE+NTS_EXPENSE+LRES_EXPENSE AS 'TOTAL_EXPENSE'
FROM #FINAL B
