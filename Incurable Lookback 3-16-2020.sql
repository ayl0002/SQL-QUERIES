...
LEFT JOIN (SELECT XX.EXCP_ID,EXCP_STS_UPDT_BY_DESC,MADE_INCUR FROM tbl2  XX
			LEFT JOIN (SELECT AA.EXCP_ID,MIN(EFF_DTTM) AS 'Made_Incur' FROM tbl2 AA
						LEFT JOIN (SELECT EXCP_ID,MAX(END_DTTM) AS 'Made_Incur' FROM tbl2 WHERE CURR_IND NOT IN ('Y') AND EXCP_STS_DESC <> 'INCURABLE' GROUP BY EXCP_ID) BB 
...
LEFT JOIN (SELECT LOAN_NBR,TAG_2_VAL,[GRP_DESC] FROM tbl3 WHERE CURR_IND = 'Y') D
ON B.LOAN_NBR  = D.LOAN_NBR
LEFT JOIN (SELECT EXCP_ID,MAX(EFF_DTTM) AS 'Date_Filter' FROM tbl2 WHERE CAST(EFF_DTTM AS DATE) <= CAST(@DateFilter AS DATE) AND EXCP_STS_DESC ='Incurable' GROUP BY EXCP_ID) E
