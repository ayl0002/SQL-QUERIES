...
SET NOCOUNT ON
SELECT A.*,C.* INTO #BASE FROM TACT_REV.DBO.NEW_FNMA_BASE_DEC_EXCP A
...
INTO #BASE1
FROM TACT_REV.DBO.NEW_FNMA_BASE_DEC_EXCP A
LEFT JOIN #BASE B
...
INTO #FINAL
FROM TACT_REV.DBO.NEW_FNMA_BASE_DEC BASE
...
LEFT JOIN (SELECT BB.LOAN_NBR,MAX(NTS_COUNT) AS 'NTS_COUNT' FROM TACT_REV.DBO.NEW_FNMA_BASE_DEC_EXCP BB
			INNER JOIN (SELECT LOAN_NBR,TAX_VENDOR,ROW_NUMBER() OVER (Partition By Loan_Nbr ORDER BY TAX_VENDOR) AS 'NTS_COUNT' FROM TACT_REV.DBO.NEW_FNMA_BASE_DEC_EXCP WHERE TAX_VENDOR = 'NTS') AA
			ON AA.LOAN_NBR = BB.LOAN_NBR  GROUP BY BB.LOAN_NBR) NTS
	ON NTS.LOAN_NBR = BASE.LOAN_NBR
LEFT JOIN (SELECT BB.LOAN_NBR,MAX(ISGN_COUNT) AS 'ISGN_COUNT' FROM TACT_REV.DBO.NEW_FNMA_BASE_DEC_EXCP BB
			INNER JOIN (SELECT LOAN_NBR,TAX_VENDOR,ROW_NUMBER() OVER (Partition By Loan_Nbr ORDER BY TAX_VENDOR) AS 'ISGN_COUNT' FROM TACT_REV.DBO.NEW_FNMA_BASE_DEC_EXCP WHERE TAX_VENDOR = 'ISGN') AA
			ON AA.LOAN_NBR = BB.LOAN_NBR GROUP BY BB.LOAN_NBR) ISGN
	ON ISGN.LOAN_NBR = BASE.LOAN_NBR
LEFT JOIN (SELECT BB.LOAN_NBR,MAX(LRES_COUNT) AS 'LRES_COUNT' FROM TACT_REV.DBO.NEW_FNMA_BASE_DEC_EXCP BB
			INNER JOIN (SELECT LOAN_NBR,HOA_VENDOR,ROW_NUMBER() OVER (Partition By Loan_Nbr ORDER BY TAX_VENDOR) AS 'LRES_COUNT' FROM TACT_REV.DBO.NEW_FNMA_BASE_DEC_EXCP WHERE HOA_VENDOR = 'Local Vendor') AA
...
FROM #FINAL B
LEFT JOIN (SELECT LOAN_NBR,MAX(SM_COUNT) AS 'Slow_Mover_Count' FROM TACT_REV.DBO.NEW_FNMA_BASE_DEC_SM GROUP BY LOAN_NBR) C
	ON B.LOAN_NBR = C.LOAN_NBR
LEFT JOIN (SELECT LOAN_NBR,MAX(Aging) AS 'Slow_Mover_Aging' FROM TACT_REV.DBO.NEW_FNMA_BASE_DEC_SM GROUP BY LOAN_NBR) D
...
LEFT JOIN (SELECT LOAN_NBR,SUM(DAYS_TO_CLOSE) AS 'EXCP_AGING' FROM TACT_REV.DBO.NEW_FNMA_BASE_DEC_EXCP GROUP BY LOAN_NBR) F
	ON B.LOAN_NBR = F.LOAN_NBR
LEFT JOIN  TACT_REV.DBO.NEW_FNMA_BASE_DEC_SM H
	ON B.LOAN_NBR = H.LOAN_NBR


--SET NOCOUNT ON SELECT B.*,D.[Tag 2],D.[Incurable Flag] FROM TACT_REV.DBO.NEW_FNMA_BASE_DEC_EXCP B LEFT JOIN SharepointData.dbo.HUDAssignLoans D ON CAST(B.Loan_Nbr AS VARCHAR) = CAST(D.[Loan Number] AS VARCHAR)

--SET NOCOUNT ON SELECT b.*,D.[Tag 2],D.[Incurable Flag] FROM TACT_REV.DBO.NEW_FNMA_BASE_DEC_SM B LEFT JOIN SharepointData.dbo.HUDAssignLoans D ON CAST(B.Loan_Nbr AS VARCHAR) = CAST(D.[Loan Number] AS VARCHAR)
