...
INNER JOIN (SELECT LOAN_NBR, MAX(EFF_DTTM) AS 'MAX_DT' FROM REVERSE_DW.[dbo].[HUD_ASGN_LOANS] WHERE EFF_DTTM <= @EOM GROUP BY LOAN_NBR) B
...
LEFT JOIN 
	(SELECT LOAN_NBR,LOAN_STATUS,MCA_PERCENT FROM REVERSE_dw.[dbo].[RM_CHAMPION_MASTER_TBL_VW] (@EOM,@EINT) WHERE LOAN_STATUS IN ('Active','Liquidated/Assigned to HU')) B
	ON A.LOAN_NBR = B.LOAN_NBR 
LEFT JOIN #BASE C
