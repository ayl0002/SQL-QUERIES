...
SET @EOM = CAST('5/31/2020' AS DATE)---CAST((SELECT MAX(BUS_PROC_DT) FROM Reverse_DW.[dbo].[HUD_ASGN_LOANS] WHERE DATEPART(Month,BUS_PROC_DT) = DATEPART(M,GETDATE()) - 1) AS DATE)

SET @BINT = CAST(REPLACE(@BOM,'-','') AS INT)
SET @EINT = CAST(REPLACE(@EOM,'-','') AS INT)

SELECT * INTO #BASE FROM Reverse_DW.[dbo].[HUD_ASGN_EXCP_EDW]
...
INTO #BOM_MCA
FROM Reverse_DW.[dbo].[RM_CHAMPION_MASTER_TBL_VW] (@BOM,@BINT) A 
...
LEFT JOIN (SELECT [EXCP_ID],MAX(Eff_DTTM) AS 'End_Excp' FROM Reverse_DW.[dbo].[HUD_ASGN_EXCP_EDW] WHERE (DOC_DESC IN ('Loss Draft') OR (ISSU_DESC IN ('Forced Placed Insurance')) OR (DOC_DESC = 'Not enough repays' AND ISSU_DESC = 'Call Center Action Required')) AND CAST(Eff_DTTM AS DATE) <= CAST(@EOM AS DATE) GROUP BY EXCP_ID) D
	ON CAST(E.[EXCP_ID] AS NVARCHAR) = D.[EXCP_ID]
LEFT JOIN (SELECT [EXCP_ID],MAX(Eff_DTTM) AS 'Open_Excp' FROM Reverse_DW.[dbo].[HUD_ASGN_EXCP_EDW] WHERE (DOC_DESC IN ('Loss Draft') OR (ISSU_DESC IN ('Forced Placed Insurance')) OR (DOC_DESC = 'Not enough repays' AND ISSU_DESC = 'Call Center Action Required')) AND Excp_Sts_Desc NOT IN ('RESOLVED','CLOSED','NOT VALID','CLOSED WITH VENDOR','INCURABLE') AND CAST(Eff_DTTM AS DATE) <= CAST(@EOM AS DATE) GROUP BY EXCP_ID) B
	ON CAST(E.[EXCP_ID] AS NVARCHAR) = B.[EXCP_ID]
LEFT JOIN (SELECT [EXCP_ID],MAX(Eff_DTTM) AS 'Close_Excp' FROM Reverse_DW.[dbo].[HUD_ASGN_EXCP_EDW] WHERE (DOC_DESC IN ('Loss Draft') OR (ISSU_DESC IN ('Forced Placed Insurance')) OR (DOC_DESC = 'Not enough repays' AND ISSU_DESC = 'Call Center Action Required')) AND Excp_Sts_Desc IN ('RESOLVED','CLOSED','NOT VALID','CLOSED WITH VENDOR','INCURABLE') AND CAST(Eff_DTTM AS DATE) BETWEEN CAST(@BOM AS DATE) AND CAST(@EOM AS DATE) GROUP BY EXCP_ID) C
	ON E.[EXCP_ID] = C.[EXCP_ID]*/
