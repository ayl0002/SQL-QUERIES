...
INTO #BASE1
from tbl1 A
LEFT JOIN (SELECT * FROM tbl2 WHERE CURR_IND = 'Y') B
	ON A.loan_nbr = b.Loan_Nbr
LEFT JOIN (SELECT EXCP.LOAN_NBR,EXCP.EXCP_ID FROM tbl3 EXCP  WHERE CURR_IND = 'Y' AND (DOC_DESC <> 'Proof of Repair' AND [EXCP_STS_DESC] = 'Resolved by ML') OR (DOC_DESC = 'Proof of Repair' AND [EXCP_STS_DESC] IN ('Resolved','Resolved by ML'))) C
	ON C.LOAN_NBR = A.LOAN_NBR
LEFT JOIN (SELECT POR1.EXCP_ID,MIN(EFF_DTTM) AS 'POR_INCUR_CLEAR' FROM tbl3 POR1
			RIGHT JOIN (SELECT EXCP_ID,MAX(EFF_DTTM) AS 'POR_INCUR' FROM tbl3 WHERE DOC_DESC = 'Proof of Repair' AND [EXCP_STS_DESC] = 'Incurable' GROUP BY EXCP_ID) POR2
...
LEFT JOIN (SELECT Loan_Nbr,[FNL_RVW_STS_DESC] FROM tbl4 WHERE CURR_IND = 'Y') F
	ON A.LOAN_NBR = F.LOAN_NBR
RIGHT JOIN (SELECT LOAN_NBR,[GRP_DESC],[TAG_2_VAL] FROM tbl5 WHERE CURR_IND = 'Y' AND ISNULL([GRP_DESC],'No Group') <> 'Grp 5 BofA GNMAs') D
...
LEFT JOIN (SELECT [EXCP_ID],MIN([EFF_DTTM]) AS 'EXCP_CLOSED' FROM tbl3 WHERE [EXCP_STS_DESC] IN ('Resolved','Resolved by ML') AND CAST(EFF_DTTM AS DATE) >= CAST('3/27/2020' AS DATE) GROUP BY EXCP_ID) BB
...
LEFT JOIN (SELECT DISTINCT EXCP_ID,[EXCP_MM_DESC] from tbl6) B
...
LEFT JOIN (SELECT Loan_Nbr, MAX([DT_SBMT_TO_HUD]) AS 'x_dt' FROM tbl7 GROUP BY LOAN_NBR) E
