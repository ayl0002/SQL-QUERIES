...
SELECT A.*,Beg_Month_MCA,	BOM_MCA_FLAG,	Beg_Month_Status,	End_Month_MCA,	EOM_MCA_FLAG,	End_Month_Status,	EOM_HUD_STS,	BuyOut_Flag

 INTO #BASE0 FROM Reverse_DW.[dbo].[RM_CHAMPION_MASTER_TBL_CURR_VW] A RIGHT JOIN ##FNMA_ROLL_BASE B ON CAST(A.LOAN_NBR AS VARCHAR) = CAST(B.LOAN_NBR AS VARCHAR)
...
SELECT tab1.*,[FNM_APRV_DTTM],[FNM_APRV_RQST_DTTM],[FNM_DND_DTTM],tab3.[HUD_STS_DESC],DATE_SUBMIT_TO_HUD,Claim_Filed--,[Jan-20_MCA%],[FEB-20_MCA%],[MAR-20_MCA%],[APR-20_MCA%],[May-20_MCA%]
,CASE 
WHEN DATEPART(MONTH,CLAIM_FILED) = 8 AND DATEPART(YEAR,CLAIM_FILED) = 2019 THEN [Aug-19_MCA%]
WHEN DATEPART(MONTH,CLAIM_FILED) = 9 AND DATEPART(YEAR,CLAIM_FILED) = 2019 THEN [Sept-19_MCA%]
WHEN DATEPART(MONTH,CLAIM_FILED) = 10 AND DATEPART(YEAR,CLAIM_FILED) = 2019 THEN [Oct-19_MCA%]
WHEN DATEPART(MONTH,CLAIM_FILED) = 11 AND DATEPART(YEAR,CLAIM_FILED) = 2019 THEN [Nov-19_MCA%]
WHEN DATEPART(MONTH,CLAIM_FILED) = 12 AND DATEPART(YEAR,CLAIM_FILED) = 2019 THEN [Dec-19_MCA%]
WHEN DATEPART(MONTH,CLAIM_FILED) = 1 AND DATEPART(YEAR,CLAIM_FILED) = 2020 THEN [Jan-20_MCA%]
WHEN DATEPART(MONTH,CLAIM_FILED) = 2 AND DATEPART(YEAR,CLAIM_FILED) = 2020 THEN [Feb-20_MCA%]
WHEN DATEPART(MONTH,CLAIM_FILED) = 3 AND DATEPART(YEAR,CLAIM_FILED) = 2020 THEN [Mar-20_MCA%]
WHEN DATEPART(MONTH,CLAIM_FILED) = 4 AND DATEPART(YEAR,CLAIM_FILED) = 2020 THEN [Apr-20_MCA%]
WHEN DATEPART(MONTH,CLAIM_FILED) = 5 AND DATEPART(YEAR,CLAIM_FILED) = 2020 THEN [May-20_MCA%]
WHEN DATEPART(MONTH,CLAIM_FILED) = 6 AND DATEPART(YEAR,CLAIM_FILED) = 2020 THEN [June-20_MCA%]
END AS 'CLAIM_FILED_MCA'
INTO #BASE
FROM #BASE0  tab1
LEFT JOIN (SELECT Loan_Nbr,[FNM_APRV_DTTM],[FNM_APRV_RQST_DTTM],[FNM_DND_DTTM] FROM Reverse_DW.[dbo].[HUD_ASGN_HUD_STS] where curr_ind ='y') tab2
	ON CAST(tab1.loan_nbr AS VARCHAR) = CAST(tab2.loan_nbr AS VARCHAR)
LEFT JOIN (SELECT Loan_Nbr,[HUD_STS_DESC] FROM Reverse_dw.[dbo].[HUD_ASGN_HUD_STS] where curr_ind = 'y'AND [HUD_STS_DESC] in ('Pkg Submitted to HUD','HUD Approval','Resubmitted to HUD','AOM Sent to HUD','HUD Approved')) tab3
	ON CAST(tab1.loan_nbr AS VARCHAR) = CAST(tab3.loan_nbr AS VARCHAR)
...
LEFT JOIN (select loan_key, max([TXN_DT]) as 'Claim_Filed' from REVERSE_DW.[dbo].[LOAN_STS_EVT_TXN] where [OLD_LOAN_STS_CD] = '0' and [NEW_LOAN_STS_CD] = '72' group by loan_key) C
	ON CAST(tab1.loan_key AS VARCHAR) = CAST(c.loan_key As VARCHAR)
/*LEFT JOIN (SELECT LOAN_NBR,[GRP_DESC] FROM Reverse_DW.DBO.HUD_ASGN_LOANS WHERE CURR_IND = 'Y') GRP
	ON CAST(tab1.LOAN_NBR AS VARCHAR) = CAST(GRP.LOAN_NBR AS VARCHAR)*/
LEFT JOIN (SELECT * FROM ##FNMA_ARCHIVE) Arch
	ON CAST(tab1.LOAN_NBR AS VARCHAR) = CAST(Arch.Loan_Nbr AS VARCHAR)

/*WHERE /*ISNULL([GRP_DESC],'No Group') <> 'Grp 5 BofA GNMAs'*/isnull(tab1.[INVESTOR],'No Pool') = 'FNMA'  AND ((tab1.MCA_PERCENT >= 97.5 AND tab1.[LOAN_STATUS] = 'Active' --AND tab3.[HUD_STS_DESC] IS NOT NULL
) or (tab1.[LOAN_STATUS] LIKE ('%Liquidated/Assigned to HU%') AND tab2.[FNM_APRV_DTTM] IS NOT NULL))*/

SELECT * FROM #BASe

--POR SCRUB--
SELECT A.*,B.Excp_ID AS 'POR_EXCP_ID',B.[DOC_DESC] AS 'POR_DOC_DESC',B.EXCP_STS_DESC AS 'POR_EXCP_STS_DESC',B.EFF_DTTM AS 'POR_EFF_DTTM',PTA_DATE,RN AS 'POR_RN'
INTO #BASE2
FROM #BASE A
...
LEFT JOIN (SELECT A.LOAN_NBR,MIN(B.EFF_DTTM) AS 'PTA_DATE' FROM #BASE A INNER JOIN Reverse_DW.[dbo].[HUD_ASGN_HUD_STS] B ON A.LOAN_NBR = B.LOAN_NBR WHERE B.EFF_DTTM >= DATE_SUBMIT_TO_HUD AND B.[HUD_STS_DESC] IN ('HUD Approval','HUD Approved') GROUP BY  A.LOAN_NBR) PTA_HUD
ON A.LOAN_NBR = PTA_HUD.LOAN_NBR
...
--MASS EXCEPTION--
SELECT A.* INTO #EXCP FROM REVERSE_DW.[dbo].[HUD_ASGN_EXCP_EDW] A INNER JOIN #BASE3 B ON CAST(A.LOAN_NBR AS VARCHAR) = CAST(B.LOAN_NBR AS VARCHAR)

--EXCEPTION SCRUB--
SELECT B.loan_nbr,B.Excp_ID,B.[DOC_DESC],B.EXCP_STS_DESC,B.EFF_DTTM,GFT_CRD_LTR_SNT_DTTM,GFT_CRD_LTR_SNT_2_DTTM,GFT_CRD_LTR_SNT_3_DTTM,SNT_FR_GFT_CRD_PROC_DTTM,SNT_TO_INSPT_VNDR_DTTM
,CASE WHEN DOC_DESC = 'TAX BILL' AND ISNULL(VNDR_DESC,'NO VENDOR') IN ('NTS','ISGN') THEN VNDR_DESC
END AS 'TAX_VENDOR'
,CASE WHEN DOC_DESC = 'HOA' AND ISNULL(VNDR_DESC,'NO VENDOR') = 'Local Vendor' THEN VNDR_DESC
END AS 'HOA_VENDOR'
,CASE WHEN  EXCP_STS_DESC IN ('RESOLVED','Resolved by ML','CLOSED WITH VENDOR') THEN 'RESOLVED'
WHEN  EXCP_STS_DESC IN ('RESOLVED','Resolved by ML','CLOSED WITH VENDOR','Closed') THEN 'RESOLVED'
WHEN  EXCP_STS_DESC = 'NOT VALID' THEN 'NOT VALID'
WHEN EXCP_STS_DESC LIKE '%Resolved%' THEN 'RESOLVED'
ELSE 'PENDING'
END AS 'EXCP_AGGREGATE'
,ROW_NUMBER () OVER (Partition by B.Loan_Nbr ORDER BY B.Excp_ID) AS 'EXCP COUNT',ISNULL(TRUE_CLOSE,FIRST_CLOSE) AS 'TRUE_CLOSE',ISNULL(TRUE_OPEN,EXCP_RQST_DTTM) AS 'TRUE_OPEN'
,CASE 
WHEN DATEDIFF(DAY,ISNULL(TRUE_OPEN,EXCP_RQST_DTTM),ISNULL(TRUE_CLOSE,FIRST_CLOSE)) >= ABS(DATEDIFF(DAY,EXCP_RQST_DTTM,FIRST_CLOSE)) THEN DATEDIFF(DAY,ISNULL(TRUE_OPEN,EXCP_RQST_DTTM),ISNULL(TRUE_CLOSE,FIRST_CLOSE))
WHEN EXCP_STS_DESC NOT IN ('RESOLVED','CLOSED','NOT VALID','CLOSED WITH VENDOR','Resolved by ML') THEN ABS(DATEDIFF(DAY,TRUE_OPEN,GETDATE()))
ELSE ABS(DATEDIFF(DAY,EXCP_RQST_DTTM,ISNULL(FIRST_CLOSE,GETDATE())))
...
SELECT A.LOAN_NBR,ISNULL(EXCP_CLOSE_COUNT,0) AS 'EXCP_CLOSE_COUNT',ISNULL(EXCP_OPEN_COUNT,0) AS 'EXCP_OPEN_COUNT',EXCP_OPEN_AGING 
INTO #EXCP_COUNT
FROM #BASE3 A 
LEFT JOIN  (SELECT LOAN_NBR,COUNT(LOAN_NBR) AS 'EXCP_CLOSE_COUNT' FROM #EXCP1 
	WHERE EXCP_STS_DESC IN ('RESOLVED','CLOSED','NOT VALID','CLOSED WITH VENDOR','Resolved by ML') GROUP BY LOAN_NBR)   B
ON A.LOAN_NBR = B.LOAN_NBR
LEFT JOIN  (SELECT LOAN_NBR,COUNT(LOAN_NBR) AS 'EXCP_OPEN_COUNT' FROM #EXCP1
	WHERE EXCP_STS_DESC NOT IN ('RESOLVED','CLOSED','NOT VALID','CLOSED WITH VENDOR','Resolved by ML') GROUP BY LOAN_NBR)   C
ON A.LOAN_NBR = C.LOAN_NBR
LEFT JOIN (SELECT LOAN_NBR,SUM(DAYS_TO_CLOSE) AS 'EXCP_OPEN_AGING' FROM #EXCP1 WHERE EXCP_STS_DESC NOT IN ('RESOLVED','CLOSED','NOT VALID','CLOSED WITH VENDOR','Resolved by ML') GROUP BY LOAN_NBR) D
...
SELECT A.LOAN_NBR,	A.FHA_CASE_NBR,	A.INVESTOR_LOAN_NBR,
CASE
WHEN STATUS_CODE = 0 THEN 'Active'
WHEN STATUS_CODE = 72 THEN 'Liquidated/Assigned to HUD'
ELSE 'Default/Non-Active'
END AS 'Agg_Status'
,A.STATUS_CODE,	A.STATUS_DESCRIPTION,	A.INVESTOR,	A.MAX_CLAIM_AMOUNT,	A.MCA_PERCENT,Beg_Month_MCA,	BOM_MCA_FLAG,	Beg_Month_Status,	End_Month_MCA,	EOM_MCA_FLAG,	End_Month_Status,	EOM_HUD_STS,	BuyOut_Flag, A.LOAN_STATUS_CATEGORY,	A.LOAN_STATUS,	A.POOL_NAME,	A.FNM_APRV_DTTM,	A.FNM_APRV_RQST_DTTM,	A.FNM_DND_DTTM,	A.HUD_STS_DESC,	A.DATE_SUBMIT_TO_HUD,--	A.POR_EXCP_ID,	A.POR_DOC_DESC,	A.POR_EXCP_STS_DESC,	A.POR_EFF_DTTM,	A.INCUR_EXCP_ID,	A.INCUR_DOC_DESC,	A.INCUR_EXCP_STS_DESC,	A.INCUR_EFF_DTTM
B.EXCP_COUNT,EXCP_CLOSE_COUNT,EXCP_OPEN_COUNT,EXCP_OPEN_AGING,POR_COUNT,INCUR_COUNT,ISNULL(C.Approved,'NA') AS 'Pre-Approval Status',ISNULL(C.BATCH,'NA') AS 'Pre-Approval Batch',ISNULL(C.Population,'NA') AS 'Pre-Approval Pop',Claim_Filed,PTA_DATE,CLAIM_FILED_MCA 
,CASE WHEN CLAIM_FILED_MCA >= 100 THEN '>=100%'
	  WHEN CLAIM_FILED_MCA >= 98 THEN '98%-99.99%'  
	  WHEN CLAIM_FILED_MCA >= 97.5 THEN '97.5%-97.99%' 
END AS 'CLAIM_FILED_MCA_BUCKET'
,CASE WHEN MCA_PERCENT >= 100 THEN '>=100%'
	  WHEN MCA_PERCENT >= 98 THEN '98%-99.99%' 	
	  WHEN MCA_PERCENT >= 97.5 THEN '97.5%-97.99%' 	
END AS 'CURRENT_MCA_BUCKET'
--,F.Excp_ID,F.[DOC_DESC],F.EXCP_STS_DESC,F.EFF_DTTM,F.TRUE_CLOSE,F.TRUE_OPEN,F.DAYS_TO_CLOSE
INTO ##NEW_FNMA_BASE
FROM #BASE3 A
LEFT JOIN (SELECT Loan_Nbr,MAX([EXCP COUNT]) AS 'EXCP_COUNT' FROM #EXCP1 GROUP BY Loan_Nbr) B
	ON A.Loan_Nbr = B.Loan_Nbr
...
/*LEFT JOIN (SELECT B.LOAN_NBR,B.Excp_ID,B.[DOC_DESC],B.EXCP_STS_DESC,B.EFF_DTTM,B.TRUE_CLOSE,B.TRUE_OPEN,B.DAYS_TO_CLOSE FROM #EXCP1 B) F
	ON A.LOAN_NBR = F.LOAN_NBR*/
LEFT JOIN (SELECT * FROM #EXCP_COUNT) F
ON A.LOAN_NBR = F.LOAN_NBR



SELECT B.loan_nbr,C.MCA_PERCENT,Beg_Month_MCA,	BOM_MCA_FLAG,	Beg_Month_Status,	End_Month_MCA,	EOM_MCA_FLAG,	End_Month_Status,	EOM_HUD_STS,	BuyOut_Flag,B.Excp_ID,B.[DOC_DESC],EXCP_AGGREGATE,B.EXCP_STS_DESC,B.EFF_DTTM,B.TRUE_CLOSE,B.TRUE_OPEN,B.DAYS_TO_CLOSE,B.TAX_VENDOR,B.HOA_VENDOR,GFT_CRD_LTR_SNT_DTTM,CASE WHEN EXCP_ID = '456762' THEN NULL ELSE GFT_CRD_LTR_SNT_2_DTTM END AS 'GFT_CRD_LTR_SNT_2_DTTM',GFT_CRD_LTR_SNT_3_DTTM,SNT_FR_GFT_CRD_PROC_DTTM,SNT_TO_INSPT_VNDR_DTTM
 ,CLAIM_FILED_MCA,CLAIM_FILED
 ,CASE WHEN CLAIM_FILED_MCA >= 100 THEN '>=100%'
	  WHEN CLAIM_FILED_MCA >= 98 THEN '98%-99.99%'  
	  WHEN CLAIM_FILED_MCA >= 97.5 THEN '97.5%-97.99%' 
END AS 'CLAIM_FILED_MCA_BUCKET'
,CASE WHEN MCA_PERCENT >= 100 THEN '>=100%'
	  ...
	  WHEN MCA_PERCENT >= 97.5 THEN '97.5%-97.99%' 	
END AS 'CURRENT_MCA_BUCKET'
 INTO ##NEW_FNMA_BASE_EXCP 
	FROM #EXCP1 B 
	LEFT JOIN #BASE C
ON B.LOAN_NBR = C.LOAN_NBR

 --SELECT * FROM ##NEW_FNMA_BASE_EXCP
 --SELECT * FROM ##NEW_FNMA_BASE
 --DROP TABLE ##NEW_FNMA_BASE_EXCP
 --DROP TABLE ##NEW_FNMA_BASE

--DROP TABLE #BASE,#BASE2,#BASE3,#EXCP,#EXCP1,#PreApproved,#EXCP_COUNT
