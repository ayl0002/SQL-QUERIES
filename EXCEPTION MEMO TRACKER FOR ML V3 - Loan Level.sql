SET NOCOUNT ON
SELECT A.Loan_Nbr,[TAG_2_VAL],[INCRBL_FLG_DESC] ,C.EXCP_ID,A.[MCA_PERCENT],[MAX_CLAIM_AMOUNT] AS 'MCA_AMT',[CURRENT_TOTAL_UPB] AS 'UPB',CASE WHEN [CURRENT_TOTAL_UPB]-[MAX_CLAIM_AMOUNT] > 0 THEN [CURRENT_TOTAL_UPB]-[MAX_CLAIM_AMOUNT] ELSE 0 END AS 'CrossOver',A.[LOAN_STATUS],B.[HUD_STS_DESC],F.[FNL_RVW_STS_DESC],POR_INCUR_CLEAR,A.POOL_NAME
INTO #BASE1
...
RIGHT JOIN (SELECT LOAN_NBR,[GRP_DESC],[TAG_2_VAL],[INCRBL_FLG_DESC] FROM REVERSE_DW.[dbo].[HUD_ASGN_LOANS] WHERE CURR_IND = 'Y' AND ISNULL([GRP_DESC],'No Group') <> 'Grp 5 BofA GNMAs') D
...
SELECT DISTINCT A.Loan_Nbr,[TAG_2_VAL],[INCRBL_FLG_DESC] ,A.[MCA_PERCENT],MCA_AMT,UPB,CrossOver,A.[LOAN_STATUS],A.[HUD_STS_DESC],A.[FNL_RVW_STS_DESC],AA.EXCP_ID,AA.DOC_DESC,AA.ISSU_DESC,EXCP_CLOSED,POR_INCUR_CLEAR,A.POOL_NAME
INTO #BASE2 
FROM #BASE1 A
...
SELECT distinct A.Loan_Nbr,A.pool_name,TAG_2_VAL,[INCRBL_FLG_DESC] ,A.[MCA_PERCENT],A.MCA_AMT,A.UPB,A.CrossOver,DATE_SUB_HUD, LOAN_STATUS,	HUD_STS_DESC,	FNL_RVW_STS_DESC,	EXCP_COUNT,EXCP_1,	EXCP_2,	EXCP_3
