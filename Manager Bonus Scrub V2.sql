...
LEFT JOIN (SELECT ADHOC.Loan_nbr,min([DT_SBMT_TO_HUD]) AS 'DT_SBMT_TO_HUD' FROM tbl1 ADHOC INNER JOIN tbl3 A ON A.Loan_Nbr = ADHOC.loan_nbr
...
LEFT JOIN (SELECT ADHOC.Loan_nbr,max([HUD_PRELIM_TTL_DNY_DT]) AS 'Denial_Dt' FROM tbl1 ADHOC INNER JOIN tbl3 A ON A.Loan_Nbr = ADHOC.loan_nbr WHERE [HUD_PRELIM_TTL_DNY_DT] IS NOT NULL AND CAST([HUD_PRELIM_TTL_DNY_DT] AS DATE)
			BETWEEN CAST(@MIN AS DATE) AND CAST(@MAX AS DATE) GROUP BY ADHOC.LOAN_NBR) C
		ON A.LOAN_NBR = C.LOAN_NBR
LEFT JOIN (SELECT ADHOC.Loan_nbr,MAX([DT_SBMT_TO_HUD]) AS 'DT_SBMT_TO_HUD' FROM tbl1 ADHOC INNER JOIN tbl3 A ON A.Loan_Nbr = ADHOC.loan_nbr
			WHERE CAST([DT_SBMT_TO_HUD] AS DATE) BETWEEN CAST(@MIN AS DATE) 
